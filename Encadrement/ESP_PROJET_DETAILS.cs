...
{
    public class ESP_PROJET_DETAILS
    {
        #region sing
        static ESP_PROJET_DETAILS instance;
        static Object locker = new Object();
        public static ESP_PROJET_DETAILS Instance
        {
            get
            {
                lock (locker)
                {
                    if (instance == null)
                    {
                        instance = new ESP_PROJET_DETAILS();
                    }

                    return ESP_PROJET_DETAILS.instance;
                }
            }

        }
        private ESP_PROJET_DETAILS() { }
        #endregion sing
        ...
        public static List<ESP_PROJET_DETAILS> GetProj(string id)
        {
            ...
            {
                mySqlConnection.Open();
                string cmdQuery = "SELECT ID_PROJET,TYPE_PROJET,NOM_PROJET,DUREE,TECHNOLOGIES,METHODOLOGIE,DESCRIPTION_PROJET,PERIODE,SEMESTRE,NIVEAU_ETUDIANT FROM   ESP_PROJET_NEW WHERE ID_PROJET = '" + id + "' AND ESP_PROJET_NEW.ID_GROUPE_PROJET IS NULL";
        ...
        [DataObjectMethod(DataObjectMethodType.Select, true)]
        public static List<ESP_PROJET_DETAILS> GetProjEtudiantClass(string id, string codecl, string typep)
        {
            List<ESP_PROJET_DETAILS> myList = null;
            using (OracleConnection mySqlConnection = new OracleConnection(AppConfiguration.ConnectionString))
            {
                mySqlConnection.Open();
                string cmdQuery = "SELECT ESP_PROJET_NEW.* FROM ESP_PROJET_NEW INNER JOIN ESP_ENCADREMENT ON ESP_PROJET_NEW.ID_PROJET = ESP_ENCADREMENT.ID_PROJET AND ESP_PROJET_NEW.ANNEE_DEB = ESP_ENCADREMENT.ANNEE_DEB AND ESP_PROJET_NEW.TYPE_PROJET = ESP_ENCADREMENT.TYPE_PROJET WHERE ESP_ENCADREMENT.ID_ET ='" + id + "' AND ESP_ENCADREMENT.CODE_CL ='" + codecl + "' AND ESP_ENCADREMENT.TYPE_PROJET = '" + typep + "' AND ESP_PROJET_NEW.ID_GROUPE_PROJET IS NULL AND ESP_ENCADREMENT.COMPORTEMENT IS NULL";
                OracleCommand myCommand = new OracleCommand(cmdQuery);
                myCommand.Connection = mySqlConnection;
                myCommand.CommandType = CommandType.Text;
        ...
        [DataObjectMethod(DataObjectMethodType.Select, true)]
        public static List<ESP_PROJET_DETAILS> GetProjEtudiant(string id, string typep)
        {
            List<ESP_PROJET_DETAILS> myList = null;
            using (OracleConnection mySqlConnection = new OracleConnection(AppConfiguration.ConnectionString))
            {
                mySqlConnection.Open();
                string cmdQuery = "SELECT ESP_PROJET_NEW.* FROM ESP_PROJET_NEW INNER JOIN ESP_ENCADREMENT ON ESP_PROJET_NEW.ID_PROJET = ESP_ENCADREMENT.ID_PROJET AND ESP_PROJET_NEW.ANNEE_DEB = ESP_ENCADREMENT.ANNEE_DEB AND ESP_PROJET_NEW.TYPE_PROJET = ESP_ENCADREMENT.TYPE_PROJET WHERE ESP_ENCADREMENT.ID_ET ='" + id + "' AND ESP_ENCADREMENT.TYPE_PROJET = '" + typep + "' AND ESP_PROJET_NEW.ID_GROUPE_PROJET IS NULL AND ESP_ENCADREMENT.COMPORTEMENT IS NULL";
                OracleCommand myCommand = new OracleCommand(cmdQuery);
                myCommand.Connection = mySqlConnection;
                myCommand.CommandType = CommandType.Text;
        ...
        public ESP_PROJET_DETAILS(OracleDataReader myReader)
        {
        ...
        public ESP_PROJET_DETAILS(string ID_PROJET, string NOM_PROJET)
        {
